...
class VGGNet(object):
  ...
  def model(self):
    """(conv3x3 -> conv3x3 -> pool2x2) block -> multiple fc."""
    
    def model_block_v1(inputs, n_filters):
      conv = tf.layers.conv2d(inputs, n_filters, (3, 3),
                              padding='SAME',
    ...
    def model_block_v2(inputs, n_filters):
      conv = tf.layers.conv2d(inputs, n_filters, (3, 3), 
                              padding='SAME',
                              kernel_regularizer=self.regularizer)
      conv = tf.layers.batch_normalization(conv, training=self.mode)
    ...
    # block1 = model_block_v1(self.distorted_images, 64)
    block1 = model_block_v1(self.inputs, 64)
    block2 = model_block_v1(block1, 128)
    block3 = model_block_v2(block2, 256)
    block4 = model_block_v2(block3, 512)
    block5 = model_block_v2(block4, 512)

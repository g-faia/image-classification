...
class VGGNet(object):
  ...
  def model(self):
    ...
    def _vgg_block_v1(inputs, n_filters):
      conv = _conv(inputs, n_filters)
      conv = _conv(conv, n_filters)
      pool = tf.layers.max_pooling2d(conv, 2, 2)
      return pool

    def _vgg_block_v2(inputs, n_filters):
      conv = _conv(inputs, n_filters)
      conv = _conv(conv, n_filters)
      conv = _conv(conv, n_filters)
      conv = _conv(conv, n_filters)
    ...
    block1 = _vgg_block_v1(self.distorted_images, 64)
    block2 = _vgg_block_v2(block1, 128)
    block3 = _vgg_block_v2(block2, 256)
